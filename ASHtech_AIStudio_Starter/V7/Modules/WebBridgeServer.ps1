    'm365_clear_office_cache' = {
         = @(
            "C:\Users\Akash Hodlur\AppData\Local\Microsoft\Office\16.0\OfficeFileCache",
            "C:\Users\Akash Hodlur\AppData\Local\Microsoft\Office\15.0\OfficeFileCache",
            "C:\Users\Akash Hodlur\AppData\Local\Microsoft\Office\OfficeFileCache"
        )
         = 0
        foreach ( in ) {
            if (Test-Path ) {
        ...
        if ( -gt 0) { "Cleared Office Document Cache from  location(s)." }
        else { "Office cache locations not found." }
    }

    'm365_office_update' = {
         = "C:\Program Files\Common Files\Microsoft Shared\ClickToRun\OfficeClickToRun.exe"
        if (Test-Path ) {
            Start-Process  -ArgumentList "scenario=Update platform=x64 culture=en-US" -Verb RunAs -Wait
            "Office update check launched."
        } else {
    ...
    'm365_office_diag' = {
         = @()
         += "=== Office 365 Deep Diagnostics ==="
         += ""

         = "C:\Program Files\Common Files\Microsoft Shared\ClickToRun\OfficeClickToRun.exe"
        if (Test-Path ) {  += "C2R Engine: Present" } else {  += "C2R Engine: Missing" }

         = Get-CimInstance -ClassName Win32_Product -Filter "Name like '%Office%'" -ErrorAction SilentlyContinue | Select-Object -First 1
        if () {  += "Office Product:  v" } else {  += "Office Product: Not detected via WMI" }

         = Get-ChildItem -Path "C:\Program Files\Microsoft Office" -Filter ospp.vbs -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
        if () {
             = cscript "" /dstatus 2>&1 | Out-String
             += "License Status: "
        } else {
             += "OSPP.VBS: Not found"
        }

         = @("Teams", "OUTLOOK", "OneDrive", "WINWORD", "EXCEL", "POWERPNT", "MSACCESS", "MSPUB")
        foreach ( in ) {
...
"
    ...
    'm365_reset_license' = {
         = @("C:\Program Files\Microsoft Office\Office16", "C:\Program Files (x86)\Microsoft Office\Office16")
         = 
        foreach ( in ) {
             = Join-Path  "ospp.vbs"
            if (Test-Path ) {  = ; break }
        ...
        if () {
             = cscript "" /dstatus 2>&1 | Out-String
            cscript "" /unpkey 2>&1 | Out-Null
            "Office license reset attempted. Result: "
        } else {
            "Office not found."
        }
    }

    'm365_disable_addins_all' = {
         = @()
         = @("WINWORD", "EXCEL", "POWERPNT", "OUTLOOK")
        foreach ( in ) {
             = @("C:\Program Files\Microsoft Office\root\Office16\.EXE", "C:\Program Files (x86)\Microsoft Office\root\Office16\.EXE")
            foreach ( in ) {
                if (Test-Path ) {
                    Start-Process  -ArgumentList "/safe" -NoNewWindow -Wait
                     += " launched in Safe Mode"
                    break
                }
            }
        }
        if (.Count -eq 0) {  += "No Office apps found" }
         -join "; "
    }

    'm365_check_prereqs' = {
         = @()
         += "=== Office 365 Prerequisites Check ==="
        if (Get-Command winget -ErrorAction SilentlyContinue) {  += "Winget: Available" } else {  += "Winget: Not available" }
         = Get-ChildItem "C:\WINDOWS\Microsoft.NET\Framework64\v4*" -Directory -ErrorAction SilentlyContinue | Select-Object -First 1
        if () {  += ".NET 4.x: " } else {  += ".NET 4.x: Not found" }
         = Get-ChildItem "C:\WINDOWS\System32\vcruntime140.dll" -ErrorAction SilentlyContinue
        if () {  += "VC++ Runtime: Present" } else {  += "VC++ Runtime: Missing" }
...
$SafeOps = @{
    ...
    # ─── Microsoft 365 / Office 365 Repair Operations ───

    'm365_repair_quick' = {
        $oct = "C:\Program Files\Common Files\Microsoft Shared\ClickToRun\OfficeClickToRun.exe"
        if (Test-Path $oct) {
            Start-Process $oct -ArgumentList "scenario=Repair platform=x64 culture=en-US RepairType=QuickRepair" -Verb RunAs -Wait
            "Office Quick Repair launched successfully!"
        } else {
            "Office Click-to-Run not found at $oct"
        }
    }
    ...
    'm365_repair_online' = {
        $oct = "C:\Program Files\Common Files\Microsoft Shared\ClickToRun\OfficeClickToRun.exe"
        if (Test-Path $oct) {
            Start-Process $oct -ArgumentList "scenario=Repair platform=x64 culture=en-US RepairType=FullRepair" -Verb RunAs -Wait
            "Office Online Repair launched successfully!"
        } else {
            "Office Click-to-Run not found at $oct"
        }
    }

    'm365_activation_check' = {
        $paths = @(
        ...
        foreach ($p in $paths) {
            if (Test-Path $p) {
                $result = cscript "$p" /dstatus 2>&1 | Out-String
                $found = $true
                return $result
            }
        }
        if (-not $found) { "Office not installed or OSPP.VBS not found." }
    }
    ...
    'm365_repair_teams' = {
        ...
        foreach ($tp in $teamsPaths) {
            if (Test-Path $tp) {
                Start-Process $tp -ArgumentList "--processStart Teams.exe" -Wait
                return "Teams launch repair initiated."
            }
        }
        "Microsoft Teams not found."
    }

    'm365_clear_teams_cache' = {
        $cachePaths = @(
        ...
        "Cleared $deleted Teams cache folders. Restart Teams to refresh."
    }
    ...
    'm365_repair_outlook' = {
        ...
        foreach ($op in $outlookPaths) {
            if (Test-Path $op) {
                Start-Process $op -ArgumentList "/repair" -Wait
                return "Outlook repair launched."
            }
        }
        "Outlook not found."
    }
    ...
    'm365_reset_onedrive' = {
        ...
        foreach ($od in $odPaths) {
            if (Test-Path $od) {
                Start-Process $od -ArgumentList "/reset" -Wait
                return "OneDrive reset initiated."
            }
        }
        "OneDrive not found."
    }
    ...
    'm365_check_onedrive' = {
        ...
        $foundPath = $null
        foreach ($od in $odPaths) {
            if (Test-Path $od) { $foundPath = $od; break }
        }
        if ($foundPath) {
            $proc = Get-Process -Name OneDrive -ErrorAction SilentlyContinue
            if ($proc) { "OneDrive is running (PID: $($proc.Id))." }
            else { "OneDrive is installed but not running. Launching..." }
        } else {
            "OneDrive is not installed."
        }
    }

    'm365_repair_office_apps' = {
        $apps = @(
            @{Name="Word"; Path="$env:ProgramFiles\Microsoft Office\root\Office16\WINWORD.EXE"},
            @{Name="Excel"; Path="$env:ProgramFiles\Microsoft Office\root\Office16\EXCEL.EXE"},
            @{Name="PowerPoint"; Path="$env:ProgramFiles\Microsoft Office\root\Office16\POWERPNT.EXE"}
        )
        $results = @()
        foreach ($app in $apps) {
            $altPath = $app.Path -replace "$env:ProgramFiles", "${env:ProgramFiles(x86)}"
            $actual = if (Test-Path $app.Path) { $app.Path } elseif (Test-Path $altPath) { $altPath } else { $null }
            if ($actual) {
                Start-Process $actual -ArgumentList "/safe" -NoNewWindow -Wait
                $results += "$($app.Name) repair launched"
            } else {
                $results += "$($app.Name) not found"
            }
        }
        $results -join "; "
    }
    ...
    'm365_diagnose' = {
        $lines = @()
        $lines += "=== Microsoft 365 Diagnostics ==="
        $lines += ""

        $oct = "C:\Program Files\Common Files\Microsoft Shared\ClickToRun\OfficeClickToRun.exe"
        if (Test-Path $oct) { $lines += "Office C2R: Installed" }
        else { $lines += "Office C2R: Not found" }

        $ospp = Get-ChildItem -Path "$env:ProgramFiles\Microsoft Office" -Filter ospp.vbs -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($ospp) { $lines += "OSPP.VBS: Found at $($ospp.FullName)" }
        else { $lines += "OSPP.VBS: Not found" }

        $teams = Get-Process -Name Teams -ErrorAction SilentlyContinue
        if ($teams) { $lines += "Teams: Running (PID: $($teams.Id))" }
        else { $lines += "Teams: Not running" }

        $outlook = Get-Process -Name OUTLOOK -ErrorAction SilentlyContinue
        if ($outlook) { $lines += "Outlook: Running (PID: $($outlook.Id))" }
        else { $lines += "Outlook: Not running" }

        $od = Get-Process -Name OneDrive -ErrorAction SilentlyContinue
        if ($od) { $lines += "OneDrive: Running (PID: $($od.Id))" }
        else { $lines += "OneDrive: Not running" }
...
function Process-Request($ctx) {
    ...
    $result = switch ($url) {
        ...
        '/api/metrics'   {
            $now = Get-Date
            if ($null -eq $Global:Cache_Metrics -or ($now - $Global:Cache_Metrics_Time).TotalSeconds -gt 5) {
        ...
        '/api/launch-gui' {
            ...
            try {
                ...
                $guiExe = Join-Path $ToolkitRoot 'UltimateToolkit_Bundle_new.exe'
                ...
                if (-not (Test-Path -LiteralPath $guiExe)) {
                    ...
                    Err "UltimateToolkit_Bundle_new.exe not found at: $guiExe"
...
try {
    ...
    'm365_activate' = {
        ...
        foreach ( in ) {
            if (Test-Path ) {
                return cscript "" /act 2>&1 | Out-String
            }
        }
        "Office not installed or OSPP.VBS not found."
    }

    'm365_uninstall_office' = {
         = "C:\Program Files\Common Files\Microsoft Shared\ClickToRun\OfficeClickToRun.exe"
        if (Test-Path ) {
            Start-Process  -ArgumentList "scenario=Install scenario=Uninstall platform=x64 culture=en-US" -Verb RunAs -Wait
            "Office uninstaller launched."
        } else {
            "Office Click-to-Run not found at "
        }
    }

    'm365_install_teams' = {
        Start-Process powershell -ArgumentList "-NoProfile -Command ""winget install --id Microsoft.Teams --accept-source-agreements --accept-package-agreements""" -Verb RunAs -Wait
        "Teams installer launched via winget."
    }

    'm365_uninstall_teams' = {
        Start-Process powershell -ArgumentList "-NoProfile -Command ""winget uninstall --id Microsoft.Teams""" -Verb RunAs -Wait
        "Teams uninstaller launched via winget."
    }

    'm365_launch_teams' = {
         = Get-Process -Name Teams -ErrorAction SilentlyContinue
        if (-not ) {
             = "C:\Users\Akash Hodlur\AppData\Local\Microsoft\Teams\Update.exe"
            if (Test-Path ) {
                Start-Process  -ArgumentList "--processStart Teams.exe"
                "Teams launching..."
            } else {
                "Microsoft Teams not found."
            }
        } else {
            "Teams is already running (PID: )."
        }
    }

    'm365_scanpst' = {
         = Get-ChildItem -Path "C:\Program Files\Microsoft Office" -Recurse -Filter "ScanPST.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
        if () {
            Start-Process .FullName
            "ScanPST launched. Please select your PST/OST file."
        } else {
            "ScanPST.exe not found. Outlook may not be installed."
        }
    }

    'm365_outlook_profile' = {
        Start-Process "control" -ArgumentList "mlcfg32.cpl"
        "Mail Control Panel opened. Create or modify Outlook profiles."
    }

    'm365_clear_outlook_cache' = {
         = "C:\Users\Akash Hodlur\AppData\Local\Microsoft\Outlook\RoamCache"
        if (Test-Path ) {
            Get-ChildItem  -File -Force -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
            "Outlook cache cleared."
        } else {
            "Outlook cache directory not found."
        }
    }
    ...
    'm365_outlook_safe' = {
        ...
        foreach ( in ) {
            if (Test-Path ) {
                Start-Process  -ArgumentList "/safe"
                return "Outlook launched in Safe Mode."
            }
        }
        "Outlook not found."
    }
    ...
    'm365_onedrive_sync' = {
         = "C:\Users\Akash Hodlur\AppData\Local\Microsoft\OneDrive\OneDrive.exe"
        if (Test-Path ) {
            Start-Process  -ArgumentList "/sync"
    ...
    'm365_onedrive_unlink' = {
         = "C:\Users\Akash Hodlur\AppData\Local\Microsoft\OneDrive\OneDrive.exe"
        if (Test-Path ) {
            Start-Process  -ArgumentList "/unlink"
            "OneDrive unlinked from this PC."
        } else {
            "OneDrive not found."
        }
    }

    'm365_onedrive_reinstall' = {
        Stop-Process -Name OneDrive -Force -ErrorAction SilentlyContinue
         = "C:\WINDOWS\System32\OneDriveSetup.exe"
        if (Test-Path ) {
            Start-Process  -ArgumentList "/uninstall" -Wait
            Start-Process 
    ...
    'm365_word_safe' = {
         = @("C:\Program Files\Microsoft Office\root\Office16\WINWORD.EXE", "C:\Program Files (x86)\Microsoft Office\root\Office16\WINWORD.EXE")
        foreach ( in ) { if (Test-Path ) { Start-Process  -ArgumentList "/safe"; return "Word launched in Safe Mode." } }
        "Word not found."
    }

    'm365_excel_safe' = {
         = @("C:\Program Files\Microsoft Office\root\Office16\EXCEL.EXE", "C:\Program Files (x86)\Microsoft Office\root\Office16\EXCEL.EXE")
        foreach ( in ) { if (Test-Path ) { Start-Process  -ArgumentList "/safe"; return "Excel launched in Safe Mode." } }
        "Excel not found."
    }

    'm365_powerpoint_safe' = {
         = @("C:\Program Files\Microsoft Office\root\Office16\POWERPNT.EXE", "C:\Program Files (x86)\Microsoft Office\root\Office16\POWERPNT.EXE")
        foreach ( in ) { if (Test-Path ) { Start-Process  -ArgumentList "/safe"; return "PowerPoint launched in Safe Mode." } }
        "PowerPoint not found."
    }

    'm365_open_sharepoint' = {
        Start-Process "https://portal.office.com/sharepoint"
        "SharePoint Online opened in your browser."
    }
}
